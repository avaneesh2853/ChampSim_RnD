...
// ooo_model_instr instr_in_focus;
...
void O3_CPU::init_instruction(ooo_model_instr arch_instr)
{
  ...
  for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
    ...
    if (arch_instr.destination_registers[i]) //true if non-zero
...
void O3_CPU::do_check_clc(uint64_t IP1)
{


  auto way = std::next(CL_CACHE.begin(), IP1 % cl_set);
    if((way->address == IP1) && (way->valid[0] == true))

     {  for (int i=0; i<2; i++)
          {
            if (way->valid[i+1])
              {
                assert(way->valid[i+1]); //placeholder for prefetch requests
                succ_ip[i] = way->ip[i];
                static_cast<CACHE*>(L1D_bus.lower_level)->prefetch_line(way->ip[i], 1, 1, true, 0);
                // cout << "CL_IP : " << way->ip[i];
              }  
...
// for (auto x : IFETCH_BUFFER) 
//   do_check_clc(x.instr_id);
...
void O3_CPU::promote_to_decode()
{
  ...
  while (available_fetch_bandwidth > 0 && !IFETCH_BUFFER.empty() && !DECODE_BUFFER.full() && IFETCH_BUFFER.front().translated == COMPLETED
    ...
    if (!warmup_complete[cpu] || IFETCH_BUFFER.front().decoded)
      ...
      do_check_clc(IFETCH_BUFFER.front().ip);
         }
    else
...
void O3_CPU::decode_instruction()
{
  ...
  for (auto x : succ_ip)
    if ((x == DECODE_BUFFER.front().ip) && (DECODE_BUFFER.front().source_memory[0]))
      hit++;
    else if ((x == DECODE_BUFFER.front().ip) && !(DECODE_BUFFER.front().source_memory[0]))  
      assert(!(DECODE_BUFFER.front().source_memory[0]));
...
{ 
  ...
  // Wriiten by Avaneesh Sai
  /*Description - Needed a structure which would hold information of loads which are critcial in nature. Criticality is defined by 
  number of cycles spent at the head of ROB. Instead of holding info in an array, I decided on using three variables which are updated
  everytime the head of ROB is updated and is a load. */

    if (IP == ROB.front().ip)
      cycles_at_head++;
    else
      {
      if((is_load) && cycles_at_head>200)
        {
        do_clc_update(IP);
        ip_counter = 2;
        prev_cl_ip = IP;
      ...
      IP = ROB.front().ip;
      is_load = bool(ROB.front().source_memory[0]);
      cycles_at_head = 1; 
      if ((is_load) && (ip_counter>0))
        {
          do_ip_update(IP,prev_cl_ip);
          ip_counter--;
        }
...
void O3_CPU::do_clc_update(uint64_t IP1)
{    
  ...
  if (way->address != IP1)
    ...
    way->valid[0] = true;
    way->address = IP1;
    // cout << "Critical load PC is " << IP1 << endl;  
    }

}

void O3_CPU::do_ip_update(uint64_t IP1, uint64_t cl_ip)
{
  auto way = std::next(CL_CACHE.begin(),(cl_ip % cl_set));
  if (way->valid[0])
  {
    if (!(way->valid[1]))
    {
...
void O3_CPU::retire_rob()
{
  ...
  while (retire_bandwidth > 0 && !ROB.empty() && (ROB.front().executed == COMPLETED)) {
    ...
    if ((current_cycle == 116921767) &&  (j<1))
      { int i = 0;
        for (auto x : CL_CACHE)
        {if (x.valid[0])
          {cout << "CL_IP " << x.address << " ";
          cout << " Succeeding load IPs are";}
        if(x.valid[1])
          cout << x.ip[0] << " ";
        if(x.valid[2])
          cout << x.ip[1] << " ";  
