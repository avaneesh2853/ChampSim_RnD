...
public:
  ...
  /* an entry must hold the two succeeding load IPs, it is indexed using the critical load IP, 
  which is characterized using the number of cycles spent by the load at the head of ROB */
  struct clc_entry_t {
  ...
  //Critical load cache
  using clc_t = std::vector<clc_entry_t>;
  const std::size_t cl_set;
  clc_t CL_CACHE{cl_set};
  ...
  void do_clc_update(uint64_t IP);
  void do_ip_update(uint64_t IP1, uint64_t cl_ip);
...
#include "ooo_cpu_modules.inc"
  ...
  O3_CPU(uint32_t cpu, double freq_scale, std::size_t dib_set, std::size_t dib_way, std::size_t dib_window, std::size_t cl_set,std::size_t ifetch_buffer_size,
      ...
      : champsim::operable(freq_scale), cpu(cpu), dib_set(dib_set), dib_way(dib_way), dib_window(dib_window), cl_set(cl_set), IFETCH_BUFFER(ifetch_buffer_size),
